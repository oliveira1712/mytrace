...
using Contracts.Contracts.MyTrace.ContractDefinition;

namespace Contracts.Contracts.MyTrace
{
    public partial class MyTraceService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, MyTraceDeployment myTraceDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<MyTraceDeployment>().SendRequestAndWaitForReceiptAsync(myTraceDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, MyTraceDeployment myTraceDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<MyTraceDeployment>().SendRequestAsync(myTraceDeployment);
        }

        public static async Task<MyTraceService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, MyTraceDeployment myTraceDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, myTraceDeployment, cancellationTokenSource);
            return new MyTraceService(web3, receipt.ContractAddress);
        }
        ...
        public MyTraceService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
        ...
        public Task<string> MintRequestAsync(string recipient, string uri)
        {
            var mintFunction = new MintFunction();
                mintFunction.Recipient = recipient;
                mintFunction.Uri = uri;
            
             return ContractHandler.SendRequestAsync(mintFunction);
        }

        public Task<TransactionReceipt> MintRequestAndWaitForReceiptAsync(string recipient, string uri, CancellationTokenSource cancellationToken = null)
        {
            var mintFunction = new MintFunction();
                mintFunction.Recipient = recipient;
                mintFunction.Uri = uri;
