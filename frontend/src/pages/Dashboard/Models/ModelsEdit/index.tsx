...
import { editModel } from "../../../../services/api/modelApi";
...
import { getColorsByModel } from "../../../../services/api/colorsSizesApi";
import { getComponentsByModel } from "../../../../services/api/componentsApi";

interface ModelsEditProps {
...
export default function ModelsEdit(props: ModelsEditProps) {
  const [file, setFile] = useState<any>();

  const inputFile = useRef<HTMLInputElement>(null);
  const formRef = useRef<FormikProps<FormValues>>(null);

  const modelEdit = useMutation(editModel, {
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ["getModels"] });
      queryClient.invalidateQueries({ queryKey: ["getOwnColors"] });
      queryClient.invalidateQueries({ queryKey: ["getOwnComponents"] });
      queryClient.invalidateQueries({ queryKey: ["getComponentsReferences"] });
  ...
  const {
    isLoading: isLoadingOwnColor,
    isError: isErrorOwnColor,
    error: errorOwnColor,
    data: ownColors,
  } = useQuery<Array<Color>, Error>(["getOwnColors"], () =>
    getColorsByModel(props.currentModel.id)
  );
  ...
  const {
    ...
    data: ownComponents,
  } = useQuery<Array<Component>, Error>(["getOwnComponents"], () =>
    getComponentsByModel(props.currentModel.id)
  );
  ...
  const ownColorsOptions: Array<Option> = [];
  ownColors?.map((color) => {
    ownColorsOptions.push({ value: color.id, label: color.color1 });
  });

  const ownComponentsOption: Array<string> = [];
  ownComponents?.map((component) => {
    ownComponentsOption.push(component.id);
  });
      ...
      content={
        <Formik
          ...
          initialValues={{
            ...
            modelPhoto: null,
            colors: ownColorsOptions,
            sizes: [],
            productionProcess: props.currentModel.stagesModelId,
            components: ownComponentsOption,
          }}
              ...
              <div className="px-5">
                ...
                <div className="grid gap-10">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <MyInput label="Id" name="id" type="text" placeholder="Model id" disabled />
                    <div
